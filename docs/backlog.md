...
| TC-07 | Implement ACTION and EVENT in `viva-fake-gige` | P1 | M | planned | Neither is implemented by the fake, so TC-02 and TC-03 have no test vehicle. Worth adding an opt-in PENDING_ACK mode in the same pass: TC-01 is covered by a raw-UDP test device in `gvcp.rs`, which exercises the layer the logic lives in but not the stack above it |
