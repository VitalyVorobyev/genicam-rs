...
| REL-03 | Ask #35, #45, #57 to retest against 0.3.0 | P0 | S | in-progress | #70's reporter (same hardware as #57) is told; #35 and #45 asked once the crates.io/PyPI publication is verified |
| REL-04 | Image acquisition fails on real hardware over Windows APIPA ([#70](https://github.com/VitalyVorobyev/viva-genicam/issues/70)) | P0 | M | in-progress | Root cause: the studio embedded backend passed the camera's IP to `Iface::from_ipv4()`, which expects a host IP. Fix in flight as [#72](https://github.com/VitalyVorobyev/viva-genicam/pull/72) (review posted); tag 0.3.1 once merged and re-validated on the reporter's JAI |
...
| ID | Task | Priority | Size | Status | Notes |
|----|------|----------|------|--------|-------|
| TC-01 | Handle `PENDING_ACK` (0x0089) — [#60](https://github.com/VitalyVorobyev/viva-genicam/issues/60) | P0 | M | done | Handled in the GVCP layer, not GenCP: U3V signals the same condition with status `0x8006` (`crates/viva-u3v/src/control.rs:29`), so the two transports do not share a representation. The controller now extends its deadline instead of retrying — a resend would risk executing a flash write twice. Bounded by `MAX_PENDING_ACKS` and `MAX_PENDING_ACK_WAIT` |
...
| TC-08 | Reconcile packet-size accounting | P1 | S | in-progress | `best_packet_size` subtracts 42 (incl. Ethernet L2, `crates/viva-gige/src/nic.rs:199-205`); the receiver and the fake subtract 36 (`crates/viva-genicam/src/stream.rs:399-407`). 6 bytes of permanent slack. **Resolved by [#72](https://github.com/VitalyVorobyev/viva-genicam/pull/72)** — `best_packet_size(mtu) = mtu` is the spec-correct accounting (`GevSCPSPacketSize` covers IP+UDP+GVSP headers) |
...
| TC-11 | `mtu()` returns a hardcoded 1500 off Linux | P2 | M | in-progress | `crates/viva-gige/src/nic.rs:173-193`. Jumbo frames can never be selected on macOS or Windows. The Windows half is resolved by [#72](https://github.com/VitalyVorobyev/viva-genicam/pull/72) (`GetIfEntry2`); macOS remains |
...
| SR-05 | Library-owned heartbeat keepalive | P0 | M | planned | Consumers lose CCP after `GevHeartbeatTimeout` (~3 s) idle. [#72](https://github.com/VitalyVorobyev/viva-genicam/pull/72) adds app-layer keepalives in camctl and the studio backend; once it merges the workspace carries three copies (`viva-service`, camctl, studio embedded backend), and this task replaces all three |
...
| ID | Task | Priority | Size | Status | Notes |
|----|------|----------|------|--------|-------|
| API-01 | Single frame-reassembly implementation | P1 | L | planned | See SR-09. [#72](https://github.com/VitalyVorobyev/viva-genicam/pull/72) adds a `#[cfg(windows)]` receive path — the review asks it to reuse `FrameAssemblyState`, but whatever lands there is one more caller to unify here |
